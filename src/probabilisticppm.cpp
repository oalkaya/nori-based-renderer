...
class Probabilisticppm : public Integrator {
public:
    /// Photon map data structure
    typedef PointKDTree<Photon> PhotonMap;

    Probabilisticppm(const PropertyList &props) {
        /* Lookup parameters */
        m_photonCount  = props.getInteger("photonCount", 100);
        m_photonRadius = props.getFloat("photonRadius", 0.0f /* Default: automatic */);
        m_alpha = props.getFloat("alpha",0.66f);
        m_emissionCount = 0;
        

    }

    virtual void preprocessPPM(const Scene *scene, int i) override {
        ...
        // Update radius
        if(i>0)
            m_photonRadius = sqrtf(((i+m_alpha)/(i+1))*m_photonRadius*m_photonRadius);
    }
...
NORI_REGISTER_CLASS(Probabilisticppm, "probabilisticppm");
NORI_NAMESPACE_END
