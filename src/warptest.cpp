...
public:
    ...
    static float mapParameter(WarpType warpType, float parameterValue) {
        if (warpType == Beckmann || warpType == MicrofacetBRDF || warpType == GTR1 || warpType == GTR2a)
    ...
    std::pair<Point3f, float> warpPoint(WarpType warpType, const Point2f &sample, float parameterValue) {
        ...
        switch (warpType) {
            ...
            case GTR2a: result << Warp::squareToGTR2a(sample,parameterValue,parameterValue); break;
            case MicrofacetBRDF: {
    ...
    void refresh() {
        ...
        m_parameterSlider->setEnabled(warpType == Beckmann || warpType == MicrofacetBRDF || warpType == UniformSphereCap || warpType == GTR1 || warpType == GTR2a);
        m_parameterBox->setEnabled(warpType == Beckmann || warpType == MicrofacetBRDF || warpType == UniformSphereCap || warpType == GTR1 || warpType == GTR2a);
    ...
    void runTest() {
        ...
        auto integrand = [&](double y, double x) -> double {
            if (warpType == None) {
                ...
                else if (warpType == GTR2a)
                    return Warp::squareToGTR2aPdf(v, parameterValue, parameterValue);
    ...
    void initializeGUI() {
        ...
        m_warpTypeBox = new ComboBox(m_window, { "None", "Disk", "Sphere", "Spherical cap", "Hemisphere (unif.)",
                "Hemisphere (cos)", "Beckmann distr.","GTR1","GTR2a", "Microfacet BRDF" });
