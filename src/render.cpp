    ...
    for (int y=0; y<size.y(); ++y) {
        for (int x=0; x<size.x(); ++x) {
            ...
            if(camera->hasChromaticAberration()){
                /* Sample a ray from the camera per channel */
                Ray3f rayR, rayG, rayB;
                Color3f valueR = camera->sampleRay(rayR, pixelSample, apertureSample, Camera::Channel_R);
                Color3f valueG = camera->sampleRay(rayG, pixelSample, apertureSample, Camera::Channel_G);
                Color3f valueB = camera->sampleRay(rayB, pixelSample, apertureSample, Camera::Channel_B);

                /* Compute the incident radiance by summing each channel value*/
                valueR *= integrator->Li(scene, sampler, rayR);
                valueG *= integrator->Li(scene, sampler, rayG);
                valueB *= integrator->Li(scene, sampler, rayB);
                value = valueR + valueG + valueB;
            }
            else{
                /* Sample a ray from the camera */
                Ray3f ray;
                value = camera->sampleRay(ray, pixelSample, apertureSample); /* Channel_ALL used as dummy variable */
...
static void renderBlockFirstPass(const Scene *scene, Sampler *sampler, ImageBlock &block, std::vector<HP> &hitpoints) {
    const Camera *camera = scene->getCamera();
    const Integrator *integrator = scene->getIntegrator();
    ...
    for (int y=0; y<size.y(); ++y) {
        for (int x=0; x<size.x(); ++x) {
            ...
            /* Compute the incident radiance */
            value *= integrator->trace_ray_forPPM(scene, sampler, ray, hitpoints, Point2f(x,y));
...
static void renderBlockPPM(const Scene *scene, Sampler *sampler, ImageBlock &block, std::vector<HP> &hitpoints) {
    const Camera *camera = scene->getCamera();
    const Integrator *integrator = scene->getIntegrator();

    Point2i offset = block.getOffset();
    Vector2i size  = block.getSize();
//    cout<<"size samples "<< size.y()*size.x()<<" hitpoints"<<hitpoints.size()<<endl;
    /* Clear the block contents */
    block.clear();
...
//        cout<<"x: "<<x<<" y:"<<y<<endl;
        ...
        /* Compute the incident radiance */
        value *= integrator->Li_forPPM(scene, sampler, ray, hp);
//        if(value[0]<Epsilon &&value[1]<Epsilon&&value[2]<Epsilon )
//            cout<<"pixel "<<hp.pixel.x()<<" "<<hp.pixel.y()<<" color "<<value<<endl;
        /* Store in the image block */
        block.put(pixelSample, value);
    }
}
...
void RenderThread::renderScene(const std::string & filename) {
    ...
    if (root->getClassType() == NoriObject::EScene) {
        ...
        if(!m_scene->getIntegrator()->isProgressivePhotonMapper())
        ...
        m_render_thread = std::thread([this,outputName] {
            ...
            std::vector<std::vector<HP>> m_hitpoints(numBlocks);
            ...
            for (uint32_t k = 0; k < numSamples ; ++k) {
                ...
                m_scene->getIntegrator()->preprocessPPM(m_scene,k);
                if(m_scene->getIntegrator()->isProgressivePhotonMapper())
                    m_scene->getIntegrator()->preprocess(m_scene);
                ...
                auto map = [&](const tbb::blocked_range<int> &range) {
                    ...
                    for (int i = range.begin(); i < range.end(); ++i) {
                        ...
                        if(k == 0 && m_scene->getIntegrator()->isProgressivePhotonMapper())
                            renderBlockFirstPass(m_scene, samplers.at(blockId).get(), block, m_hitpoints[blockId]);
                        else if(m_scene->getIntegrator()->isProgressivePhotonMapper())
                            renderBlockPPM(m_scene, samplers.at(blockId).get(), block, m_hitpoints[blockId]);
                        else
